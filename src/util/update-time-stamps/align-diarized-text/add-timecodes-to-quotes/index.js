...
import align from '../alignment-from-stt/index.js';
...
function addTimecodesToQuotes(linesWithSpeaker, sttTranscript) {
    ...
    const sanitizedLinesWithSpeaker = linesWithSpeaker;
    ...
    console.log(resultAlignedSttWords[resultAlignedSttWords.length-1])
    // // line level alignement 
    const reAlignedTranscription = align(humanCorrectedTranscription, resultAlignedSttWords, 'text', true);
    ...
    const quotesWithTiems = sanitizedLinesWithSpeaker.map((quote) => {
        const match = reAlignedTranscriptionWithWords.find((elem) => {
            // console.log(' console.log(quote.text, elem.text)')
...
export default addTimecodesToQuotes;